...
FunctionType* extractTypeOfFunction(ASTNode* node){
    ...
    for(i=0; i<no_of_inputs; i++,inputs=inputs->next){
        if(inputs->sc->rs->type == typeNode){
            if(strcmp(inputs->sc->rs->node.typeNode.lexeme, "BOOLEAN"))
                inputType[i] = boolean;
            else if(strcmp(outputs->sc->rs->node.typeNode.lexeme, "REAL"))
                outputType[i] = real;
            else if(strcmp(outputs->sc->rs->node.typeNode.lexeme, "INTEGER"))
